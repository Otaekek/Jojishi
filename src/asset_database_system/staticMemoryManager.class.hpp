...
public:
	...
	static void 						set_asset_state(staticMemoryManager::E_ASSET_TYPE state, uint64_t ref);
	static void							asset_loaded(uint64_t ref, E_ASSET_TYPE type);
...
private:
	...
	static assetSystem::e_asset_state											data_status[MAXREF];
	static void															*ref_to_ptr[MAXREF];
	static uint64_t	 													referencer;
	static uin32_t														count;
};
